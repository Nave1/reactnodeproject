...
const UpdateCardSelection = () => {
  // in: Retrieves cards and deleteCard function from CardsContext.
  // out: Sets up local state for deletion confirmation and the current user's role.
  const { cards, deleteCard } = useContext(CardsContext);
  const [deleteConfirm, setDeleteConfirm] = useState(null);
  const [userRole, setUserRole] = useState(null);

  // in: Runs once when the component mounts.
  // out: Retrieves the user cookie, parses it, and updates the userRole state.
  useEffect(() => {
